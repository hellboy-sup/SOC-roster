...
const ADMIN_ACTIONS = new Set([
  'editShift', 'processRequest', 'publishRoster', 'getLeaveLog'
]);
...
const USER_ACTIONS = new Set([
  ...
  'getRoster', 'getEmployeeMaster', 'getAvailableMonths', 'checkAdminStatus'
]);
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // ── 1. Verify identity from the access token ──────
      const authHeader = request.headers.get('Authorization') || '';
      const token = authHeader.replace('Bearer ', '').trim() || body.token;
      const callerEmail = await verifyAccessToken(token);
      ...
      if (action === 'getRoster')         return await handleGetRoster(body, saToken, env);
      if (action === 'getEmployeeMaster') return await handleGetEmployeeMaster(saToken, env);
      if (action === 'getAvailableMonths')return await handleGetAvailableMonths(saToken, env);
      if (action === 'getLeaveLog')       return await handleGetLeaveLog(saToken, env);
      if (action === 'checkAdminStatus')  return await handleCheckAdminStatus(callerEmail, env);
...
// ═══════════════════════════════════════════════════════════════
//  IDENTITY — Access token verification
// ═══════════════════════════════════════════════════════════════
async function verifyAccessToken(token) {
  ...
  // aud check is skipped since we are verifying the access token directly
...
async function handleGetRoster(body, saToken, env) {
  const { month } = body;
  if (!month) return json({ ok: false, error: 'Missing month.' }, 400);
  const data = await sheetsRead(saToken, env, `${month}!A4:AG22`);
  return json({ ok: true, data });
}

async function handleGetEmployeeMaster(saToken, env) {
  const data = await sheetsRead(saToken, env, 'Employee_Master!A3:F25');
  return json({ ok: true, data });
}

async function handleGetAvailableMonths(saToken, env) {
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${env.SHEET_ID}?fields=sheets.properties.title`;
  const r = await fetch(url, { headers: { Authorization: `Bearer ${saToken}` } });
  if (!r.ok) return json({ ok: false, error: 'Failed to fetch sheet metadata' }, 502);
  const data = await r.json();
  const titles = data.sheets.map(s => s.properties.title);
  const available = titles.filter(t => MONTH_ORDER.some(m => t.startsWith(m)));
  return json({ ok: true, data: available });
}

async function handleGetLeaveLog(saToken, env) {
  const data = await sheetsRead(saToken, env, 'Leave_Swap_Log!A3:L500');
  return json({ ok: true, data });
}

async function handleCheckAdminStatus(callerEmail, env) {
  const isAdmin = await checkIsAdminKV(callerEmail, env);
